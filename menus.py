...
class StartMenu(engine.gui.Menu):
    ...
    def __init__(self, x, y, options, game_state):
        engine.gui.Menu.__init__(self, x, y, options)
        self.options_menu = OptionMenu(52, 144, ['TOGGLE FULLSCREEN/WINDOW', 'CONTROLS', 'BACK'])
...
class PauseMenu(engine.gui.Menu):
    ...
    def __init__(self, x, y, options):
        engine.gui.Menu.__init__(self, x, y, options)
        self.options_menu = OptionMenu(52, 16, ['TOGGLE FULLSCREEN/WINDOW',
                                 'CONTROLS', 'BACK'])
...
class OptionMenu(engine.gui.Menu):
    ...
    def __init__(self, x, y, options):
        engine.gui.Menu.__init__(self, x, y, options)
        self.controls_menu = ControlsMenu(84,128, ['VIEW DEFAULT','RECONFIGURE',
...
class ControlsMenu(engine.gui.Menu):
    ...
    def __init__(self, x, y, options):
        ...
        self.reset_dialog = engine.gui.DialogBox(16, 160)
        self.reset_dialog.set_text(['Controls reset to default!'])
        self.config_screen = ConfigDialogBox(16, 160)
    ...
    def check_gamepad(self):
        ...
        if game.input_manager.gamepad_name is not None:
            self.gamepad_text = game.input_manager.gamepad_name + '\nDetected'
        else:
...
class ConfigDialogBox(engine.gui.DialogBox):
    ...
    def __init__(self, x, y):
        engine.gui.DialogBox.__init__(self, x, y)
    ...
    def handle_input(self):
        ...
        if self.page < 6:  # buttons left to configure
            if not self.page_done:  # only get button presses when the page is done
                pygame.event.clear()
            new_value = game.input_manager.config_handle_input() # get input, assign to new_value
            ...
            pressed = game.input_manager.config_handle_input()  # get any key pressed
            if pressed:
